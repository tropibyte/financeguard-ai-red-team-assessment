...
def fgsm_attack(image, epsilon, data_grad):
    ...
    """
    # FGSM: x_adv = x + epsilon * sign(grad_x(loss)), clamped to valid pixels.
...
def evaluate_fgsm(model_path, test_dir, epsilon):
    ...
    for image, label in loader:
        ...
        # 1. Track the input so autograd populates image.grad
        ...
        # 3-5. Loss w.r.t. the true label, then backprop to the input
        ...
        # 7-8. Re-classify the perturbed image (no grad needed)
        with torch.no_grad():
        ...
        # 9. A "flip" is a sample the model got right until it was perturbed
